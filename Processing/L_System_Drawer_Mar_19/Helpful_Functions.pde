...
class Triangle {
  ...
  void setWindowProds(ArrayList<String> WP) {
    ...
    WindowProds = WP;
    //startNode.WindowProds = WindowProds;
    //print(WindowProds);
  }
  ...
  void display() {
    int fill = col;
    if ((over || press) && (movable && !(startNode.activated || startNode.afterActivated()))) { // false when the startnode or any of the following nodes are activated
      fill = col + 50;
    }
    w.stroke(0);
    w.strokeWeight(1);
